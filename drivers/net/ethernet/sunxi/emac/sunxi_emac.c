...
static inline ssize_t i2c_memory_read(struct i2c_client *client, char *buf, off_t offset, size_t count);
static inline ssize_t i2c_memory_write(struct i2c_client *client, const char *buf, off_t offset, size_t count);
...
struct i2c_client *sunxi_emac_i2c_client = NULL;
struct memory_accessor *sunxi_emac_mem_acc = NULL;

void get_mac_addr(struct memory_accessor *mem_acc, void *context)
 {
	sunxi_emac_mem_acc = mem_acc;
 }
...
static struct i2c_board_info __initdata sunxi_emac_i2c_devices = {
...
static int wemac_set_mac_address(struct net_device *dev, void *p)
{
	...
	printk(KERN_INFO "setting mac address\n");

	if (netif_running(dev)) {
		printk(KERN_ERR "not setting invalid mac address dev busy\n");
		return -EBUSY;
	}

	if (!is_valid_ether_addr(addr->sa_data)) {
		dev_err(&dev->dev, "not setting invalid mac address %pM\n", addr->sa_data);
		printk(KERN_ERR "not setting invalid mac address %pM\n", addr->sa_data);
		return -EADDRNOTAVAIL;
	}

	if( sunxi_emac_i2c_client ) {
		sprintf( tmp, "%pM", addr->sa_data );
		i2c_memory_write(sunxi_emac_i2c_client, tmp, 0, sizeof(tmp));
		printk(KERN_INFO "MAC address write on EEPROM : %s \n", p );
	}
	else {
		printk(KERN_ERR "MAC address on EEPROM :open erro\n" );
		return -EADDRNOTAVAIL;
	}
...
static inline ssize_t i2c_memory_read(struct i2c_client *client, char *buf, off_t offset, size_t count)
{
	struct memory_accessor *macc = sunxi_emac_mem_acc;
	
	if (macc == NULL || macc->read == NULL)
		return -ENODEV;
	return (macc->read)(macc, buf, offset, count);
}
...
static inline ssize_t i2c_memory_write(struct i2c_client *client, const char *buf, off_t offset, size_t count)
{
	...
	if (macc == NULL || macc->write == NULL)
		return -ENODEV;
	return (macc->write)(macc, buf, offset, count);
}
...
static void wemac_get_macaddr(wemac_board_info_t *db)
{
	...
	if( sunxi_emac_i2c_client ) {
		i2c_memory_read(sunxi_emac_i2c_client, tmpr, 0, 20);
		tmpr[17] = 0;
		printk(KERN_INFO "MAC address on EEPROM : %s \n", p );
	}
	else {
		printk(KERN_ERR "MAC address on EEPROM :open erro\n" );
	}
	...
	printk(KERN_INFO "MAC address after decode : %pM \n", ndev->dev_addr );
	...
	if (!is_valid_ether_addr(ndev->dev_addr)){
		printk(KERN_ERR "Invalid MAC address. Calcul random one !! \n" );
...
static int __init wemac_init(void)
{
	...
	sunxi_emac_i2c_client = i2c_new_device(adap, &sunxi_emac_i2c_devices);
...
static void __exit wemac_cleanup(void)
{
	...
	if( sunxi_emac_i2c_client )
		i2c_unregister_device(sunxi_emac_i2c_client);
