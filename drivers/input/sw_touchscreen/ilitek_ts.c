...
#error ts touche frimware compile
...
static int ilitek_i2c_process_and_report(void)
{
	...
	if(ilitek_ts->protocol_ver & 0x200){
		...
		// read i2c data from device
		//for( j=0;j<(ilitek_ts->x_ch*ilitek_ts->y_ch);j++)
		while( __gpio_get_value(CTP_IRQ_NUMBER) == 0 )
		{
			...
			if (Report_Flag!=0){
				...
				y = (int)buf[3] + ((int)buf[4] * 256);
				dprintk(DEBUG_INIT,"%d Point[%d]= %d, %d --- ",j, buf[0]&0x3F, x, y);
				x = (int)buf[5] + ((int)buf[6] * 256);
				y = (int)buf[7] + ((int)buf[8] * 256);
				dprintk(DEBUG_INIT,"%d, %d\n", x, y);
			}
			switch (tp_id)
			{
